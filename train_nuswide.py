...
load_time = time.time()
train_img_feats, train_txt_vecs, train_labels, test_img_feats, test_txt_vecs, test_labels = load_nuswide()
...
label_mask = np.zeros((total_num, 1), dtype=np.float32)
...
# img loss
theta_v = 1.0 / 8 * tf.matmul(emb_v, tf.transpose(emb_w))
...
# quantization loss
quanti_ones = np.ones((FLAGS.hash_bit, 1), dtype=np.float32)
quantization_img = tf.reduce_sum(tf.multiply(tf.matmul(tf.pow((emb_v - hash_emb_v), 2), quanti_ones), label_mask))
# balance loss
balance_img = tf.reduce_sum(tf.pow(tf.matmul(tf.transpose(emb_v), label_mask), 2))
img_loss = neg_likely_v + 0.01 * balance_img

#
# # txt loss
theta_w = 1.0 / 8 * tf.matmul(emb_w, tf.transpose(emb_v))
...
# quantization loss
quantization_txt = tf.reduce_sum(tf.multiply(tf.matmul(tf.pow((emb_w - hash_emb_w), 2), quanti_ones), label_mask))
# balance loss
balance_txt = tf.reduce_sum(tf.pow(tf.matmul(tf.transpose(emb_w), label_mask), 2))
txt_loss = neg_likely_w + balance_txt
...
img_op = img_optimizer.minimize(img_loss, var_list=img_vars + siamese_vars)

# txt optimizer
txt_optimizer = tf.train.AdamOptimizer(learning_rate=FLAGS.lr_txt)
txt_vars = txt_model.vars
txt_op = txt_optimizer.minimize(txt_loss, var_list=txt_vars + siamese_vars)

# hash optimizer
# hash_loss = 0.01 * balance_img + balance_txt
# hash_optimizer = tf.train.AdamOptimizer(learning_rate=FLAGS.lr_domain)
# hash_op = hash_optimizer.minimize(hash_loss, var_list=siamese_vars)

# combine optimize
# lr = tf.placeholder(tf.float32)
# lr_all = np.linspace(np.power(10, -3.0), np.power(10, -6.), FLAGS.epochs)
...
for epoch in range(FLAGS.epochs):
    ...
    # op_img, op_txt = sess.run([img_op, txt_op], feed_dict=feed_dict)
    likely_loss_v, hash_likely_loss_v, q_img, temp_emb_v, balance_v = sess.run([neg_likely_v, hash_neg_likely_v, quantization_img, emb_v, balance_img], feed_dict=feed_dict)
    likely_loss_w, hash_likely_loss_w, q_txt, temp_emb_w, balance_w = sess.run([neg_likely_w, hash_neg_likely_w, quantization_txt, emb_w, balance_txt], feed_dict=feed_dict)
    ...
    # Print results
    print("Epoch:", '%04d' % (epoch + 1), "neg_v:", "{:.5f}".format(likely_loss_v), "quantization_img:", "{:.5f}".format(q_img), "balance_img:", "{:.5f}".format(balance_v),
          "neg_w:", "{:.5f}".format(likely_loss_w), "quantization_txt:", "{:.5f}".format(q_txt), "balance_txt:", "{:.5f}".format(balance_w),
...
print('recall_i2t')
print(recall_i2t)
print("--------------continuous PR-curve------------")
c_precision_i2t, c_recall_i2t = calc_precision_recall(test_img_feats_trans, train_txt_vecs_trans, test_labels, train_labels)
c_precision_t2i, c_recall_t2i = calc_precision_recall(test_txt_vecs_trans, train_img_feats_trans, test_labels, train_labels)
print('c_precision_t2i')
print(c_precision_t2i)
print('c_recall_t2i')
print(c_recall_t2i)
print('c_precision_i2t')
print(c_precision_i2t)
print('c_recall_i2t')
print(c_recall_i2t)

# result_save_path = './data/'+FLAGS.dataset+'/'+'result/'+datetime.now().strftime("%d-%h-%m-%s")+'_'+str(FLAGS.hash_bit)+'_bit_result.pkl'
result_save_path = './data/'+FLAGS.dataset+'/'+'result/'+'test3'+'_'+str(FLAGS.hash_bit)+'_bit_result.pkl'
result = dict()
...
result['c_precision_i2t'] = c_precision_i2t
result['c_precision_t2i'] = c_precision_t2i
result['c_recall_i2t'] = c_recall_i2t
result['c_recall_t2i'] = c_recall_t2i
